...
- **Storage**:
  ...
  - `~/NAS`: 915.82 GiB (Ext4, relatime)
- **OS**: Fedora Linux 44 (Workstation Edition)
- **Primary Role**: Local storage, media streaming, and Docker host for internal services.
- **Docker**: 24 containers; media & indexer stack, local dev databases, DNS sinkhole, internal sync, container agents, and application backends.

### Cloud VPS: `notakrista`

- **Host**: Cloud VPS (KVM/QEMU Standard PC)
- **CPU**: AMD EPYC (8 vCPUs) @ 2.50 GHz
- **RAM**: 23.47 GiB
- **Swap**: Disabled (swapfile.img commented out in `/etc/fstab`)
- **TCP**: `cubic` + `fq_codel` (should be `bbr` + `fq`)
- **Storage**: 193.83 GiB (Ext4, relatime)
- **Overlay**: Pangolin tunnel active (`pangolin` interface on overlay subnet)
- **OS**: Debian GNU/Linux 12 (bookworm)
- **Primary Role**: Public-facing gateway, reverse proxy, and external-facing service runner.
- **Docker**: 23 containers; Traefik, Pangolin zero-trust overlay, media archive, internal workflow & admin utilities, production databases, and service agents.
...
- [x] **Add `noatime` to monolith data mounts**: done 2026-08-13: `/`, `/home`, and `~/NAS` all confirmed `noatime` via `/etc/fstab` edit + live remount
...
- Applied and persisted `vm.swappiness=1`, `bbr`+`fq`, and `noatime` on `/`, `/home`, `~/NAS` (see checklist above).
...
- Identified existing Pangolin overlay on VPS; can be extended to monolith.
