...
An `error in libcrypto: unsupported` failure on a key that is known to work elsewhere is not proof the key itself is broken. Before assuming key corruption or regenerating anything, check whether the current shell's `SSH_AUTH_SOCK` is actually pointed at the Bitwarden agent. Different terminal environments (e.g. Git Bash vs. a properly configured Windows terminal) can have inconsistent agent wiring even on the same machine, so a failure in one shell and success in another is a strong signal the agent socket — not the key — is the problem.
